...
func main() {
	fmt.Println("DirectOutput SDK Only Test")
	fmt.Println("==========================")
	...
	err = sdk.Initialize("SDK Only Test")
	if err != nil {
	...
	err = sdk.AddPage(deviceHandle, 1, "SDK Test Page", fip.FLAG_SET_AS_ACTIVE)
	if err != nil {
	...
	// Save the test image
	err = sdk.SaveImageAsPNG(testImage, "sdk_only_test_image.png")
	if err != nil {
		log.Printf("Warning: Failed to save test image: %v", err)
	} else {
		fmt.Println("   ✓ Test image saved as 'sdk_only_test_image.png'")
	}
	...
	err = sdk.SaveImageAsPNG(colorImage, "sdk_only_color_bars.png")
	if err != nil {
		log.Printf("Warning: Failed to save color image: %v", err)
	} else {
		fmt.Println("   ✓ Color image saved as 'sdk_only_color_bars.png'")
	}
	...
	err = sdk.SaveImageAsPNG(textImage, "sdk_only_text_pattern.png")
	if err != nil {
		log.Printf("Warning: Failed to save text image: %v", err)
	} else {
		fmt.Println("   ✓ Text image saved as 'sdk_only_text_pattern.png'")
	}
	...
	err = sdk.SaveImageAsPNG(testFileImage, "sdk_only_test_fip_image.png")
	if err != nil {
		log.Printf("Warning: Failed to create test file: %v", err)
	} else {
		// Try to load and send the file
		err = sdk.SetImageFromFile(deviceHandle, 1, 0, "sdk_only_test_fip_image.png")
		if err != nil {
	...
	fmt.Println("\n✅ DirectOutput SDK Only Test Completed!")
	fmt.Println("\nGenerated test images:")
	fmt.Println("  - sdk_only_test_image.png (Simple test)")
	fmt.Println("  - sdk_only_color_bars.png (Color bars)")
	fmt.Println("  - sdk_only_text_pattern.png (Text pattern)")
	fmt.Println("  - sdk_only_test_fip_image.png (File test)")
