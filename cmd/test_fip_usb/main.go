...
func main() {
	fmt.Println("USB FIP Image Sender")
	fmt.Println("=====================")

	// Create FIP USB instance
	fipUSB := fip.NewFIPUSB()

	// Connect to FIP device
	fmt.Println("1. Connecting to FIP device via USB...")
	err := fipUSB.Connect()
	if err != nil {
		log.Fatalf("Failed to connect to FIP: %v", err)
	}
	defer fipUSB.Disconnect()

	fmt.Println("✓ Connected to FIP device via USB")

	// Get device info
	deviceInfo, err := fipUSB.GetDeviceInfo()
	if err != nil {
	...
	err = fipUSB.SaveImageAsPNG(testImage, "fip_usb_test_image.png")
	if err != nil {
		log.Printf("Warning: Could not save test image: %v", err)
	} else {
		fmt.Println("✓ Test image saved as 'fip_usb_test_image.png'")
	}

	// Send image to FIP
	fmt.Println("\n3. Sending image to FIP via USB...")
	err = fipUSB.SendImage(testImage)
	if err != nil {
		log.Fatalf("Failed to send image to FIP: %v", err)
	}

	fmt.Println("✓ Image sent to FIP successfully via USB!")

	// Test LED control
	fmt.Println("\n4. Testing LED control via USB...")
	for i := 0; i < 6; i++ {
		fmt.Printf("Setting LED %d ON\n", i)
		err = fipUSB.SetLED(i, true)
		if err != nil {
	...
	eventChan, err := fipUSB.ReadButtonEvents()
	if err != nil {
	...
	err = fipUSB.SendImage(colorBarsImage)
	if err != nil {
	...
	err = fipUSB.SendImage(gradientImage)
	if err != nil {
	...
	// Test image 3: Text
	textImage := createTextImage("FIP USB TEST")
	err = fipUSB.SendImage(textImage)
	if err != nil {
	...
	fmt.Println("\n✓ USB FIP test completed successfully!")
	fmt.Println("\nNext Steps:")
...
func createTestImage() image.Image {
	...
	// Test text areas
	drawText(img, "FIP USB", 160, 60, color.RGBA{255, 255, 255, 255})
